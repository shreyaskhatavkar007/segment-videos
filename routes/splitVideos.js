...
router.post('/api/process-interval', async function(req, res, next) {
  ...
  if (intervals <= 0) {
    return res.render("splitvideos", {serverError:"Interval_Duration must be greater than zero"});
  }
  internal_videos.push(
    {
      "video_url" : 'public/static/outputVideo' + initialCount + '.mp4',
  ...
  var download = function(url, dest) {
    ...
    try {
      https.get(url, function(response) {
        ...
        return res.render("splitvideos", {serverError:"Invalid URL"});
      });
    } catch(e) {
      return res.render("splitvideos", {serverError:e.message});
    }
  };
  ...
  .on('end', function(err) {
    if(!err) {
      ...
      if (initialCount <= numOfVideos) {
        ...
        return res.render("splitvideos", {internal_videos:internal_videos});
      }
    } else {
      return res.render("splitvideos", {serverError:"Invalid URL"});
